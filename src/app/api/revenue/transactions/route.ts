...
export async function GET(request: NextRequest) {
  ...
  const result = await auth.supabaseAdmin
    .from('payments')
    .select('id, created_at, approved_at, buyer_user_id, amount, refunded_amount, status, payment_type, target_type', { count: 'exact' })
  ...
  const [stigmaResult, splitResult] = await Promise.all([
    buyerIds.length ? auth.supabaseAdmin.from('stigmas').select('user_id, email, user_name').in('user_id', buyerIds) : { data: [] as StigmaRow[] },
    paymentIds.length ? auth.supabaseAdmin.from('payment_splits').select('payment_id, rhizomes(site_label)').in('payment_id', paymentIds) : { data: [] as SplitRow[] },
  ]);
